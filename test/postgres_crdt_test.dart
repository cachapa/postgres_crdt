...
Future<void> main() async {
  ...
  group('Basic', () {
    ...
    test('Changeset', () async {
      await insertUser(crdt, 1, 'John Doe');
      final changeset = await crdt.getChangeset();
      expect((changeset['users']!.first.data as Map<String, Object?>)['name'],
          'John Doe');
    });
    ...
    test('Merge changeset with multiple tables', () async {
      ...
      expect(
        outputChangeset['other_users']!.first.hlc,
        hlc,
      );
      expect(result2.last, [9, 'John Doe 9']);
      expect(
        outputChangeset['other_users']!.last.hlc,
        hlc,
      );
    });
  });
...
Future<void> insertPurchase(PostgresCrdt crdt, int id, int userId, int price) =>
    crdt.execute(
      r'''
